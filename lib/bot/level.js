...
exports.run = (id, username, msg) => {
  ...
  function callback(profil) {
    profil === "not_found" ? create_profil(id) : add_xp(id, profil.xp, profil.level);
  }
...
function needToLevelUp(xp,level) {
  ...
  function add_xp(id, xp, level) {
    ...
    if(needToLevelUp(xp_to_add+xp, level)) msg.channel.send(`<@!${msg.author.id}> Gg, you're up to level **${level+1}** !`);
