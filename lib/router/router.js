  ...
  function callback(doc) {
    if (req.session.username) {
      res.render("login", {
        ...
        stats : {
          guilds_count:doc.servers_count,
...
router.post("/guilds/:guild_id/set-prefix", (req, res) => {
  if (!req.session.username) return res.redirect("/login_redirect");
  const guild_id = req.params.guild_id,
    prefix = req.body.new_prefix;
  ...
  fetch("https://discordapp.com/api/users/@me/guilds", {
    ...
    .then(data => {
      for (var i = 0; i < data.length; i++) {
        console.log(data[i])
        if (data[i].id === guild_id) {
          console.log("guild trouvé")
          let isAdmin = new Discord.Permissions(data[i].permissions).any(8);
          if (isAdmin === false) return res.redirect("/guilds");
          server.set_prefix(guild_id, prefix);
          res.redirect("/guilds/" + guild_id);
        }
      }
    });
});
